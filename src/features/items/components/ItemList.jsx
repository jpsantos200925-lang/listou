...
function ItemRow({ item, onToggle, onDelete, onEditRequest }) {
  ...
  return (
    <div className={`${styles.entry} ${removing ? styles.removing : ''}`}>
      <div className={`${styles.swipeWrap} ${offset !== 0 ? styles.swiped : ''}`}>
        ...
        >
          <label
            ...
            onClick={(e) => { if (offsetRef.current !== 0) { e.preventDefault(); setOffset(0) } }}
          >
            <input
              type="checkbox"
          ...
          <button
            className={`bg-transparent border-none cursor-pointer p-[0.35rem] text-[#555] rounded-[5px] flex items-center justify-center transition-[color,background] duration-150 hover:text-[#aaa] hover:bg-white/5 shrink-0 ${item.checked ? '' : ''}`}
      ...
      {item.is_online_purchase && (
        <PriceSearchSection itemId={item.id} itemName={item.name} />
      )}
    </div>
  )
}
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-5 p-12">
        <div className="w-[72px] h-[72px] rounded-[18px] bg-primary-12-item border-[1.5px] border-primary-25 flex items-center justify-center overflow-hidden mb-2 text-[1.8rem]">
          {list?.logo_url ? (
            <img src={list.logo_url} alt={list.name} className="w-full h-full object-cover rounded-[10px]" />
          ) : list ? (
        ...
        <span className="block mt-2.5 text-xs text-text-3 tracking-[.04em]">Toque no + para adicionar</span>
      </div>
    )
  }
      ...
      {pending.length > 0 && (
        <>
          <div className={sectionHeaderCls}>
            ...
            <span className={sectionCountCls}>{pending.length} {pending.length === 1 ? 'item' : 'itens'}</span>
          </div>
      ...
      {done.length > 0 && (
        <>
          <div className={sectionHeaderCls} style={{ marginTop: pending.length ? 20 : 0 }}>
            ...
            <span className={sectionCountCls}>{done.length} {done.length === 1 ? 'item' : 'itens'}</span>
          </div>
          <div className="flex flex-col gap-4">
            {done.map((item) => (
              <ItemRow key={item.id} item={item} onToggle={onToggle} onDelete={onDelete} onEditRequest={onEditRequest} />
