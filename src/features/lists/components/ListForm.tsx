...
const PRESETS: Preset[] = [
  ...
  },
  {
    ...
    title_color: '#cbd5e1',
    label_color: '#94a3b8',
    item_bg_color: '#0f1214',
  },
]

const DEFAULTS: ListFormData = {
  name: '',
  slug: '',
  primary_color: '#22c55e',
  secondary_color: '#16a34a',
...
const inputCls =
  'bg-[#111] border border-[#2a2a2a] rounded-lg text-[#f0f0f0] py-[0.6rem] px-3 text-[0.9rem] w-full outline-none focus:border-primary transition-[border-color]'
const labelCls = 'text-[0.75rem] text-label uppercase tracking-[0.05em]'

type ColorKey = keyof Omit<ListFormData, 'name' | 'slug'>

interface ListFormProps {
  open: boolean
  onClose: () => void
  onSave: (form: ListFormData, logoFile: File | null) => Promise<void>
  initial?: List | null
}

export default function ListForm({ open, onClose, onSave, initial }: ListFormProps) {
  ...
  useEffect(() => {
    if (!open) return
    const values: ListFormData = initial
      ? {
          name: initial.name,
          slug: initial.slug,
          ...
          title_color: initial.title_color,
          label_color: initial.label_color,
          item_bg_color: initial.item_bg_color,
        }
      : DEFAULTS
    reset(values)
    setLogoFile(null)
  ...
  const setColor = (key: ColorKey, val: string) => {
    setActivePreset(null)
    setValue(key, val)
  }
  ...
  const applyPreset = (preset: Preset) => {
    ...
    setValue('bg_color', preset.bg_color)
    setValue('font_color', preset.font_color)
    setValue('title_color', preset.title_color)
    setValue('label_color', preset.label_color)
    setValue('item_bg_color', preset.item_bg_color)
  }
  ...
  const colorFields: Array<{ label: string; key: ColorKey }> = [
  ...
  return (
    ...
    >
      <div
        className="bg-[#1a1a1a] rounded-[20px_20px_0_0] px-5 py-6 pb-8 w-full max-w-[480px] max-h-[90vh] overflow-y-auto"
        onClick={e => e.stopPropagation()}
      >
        <h2 className="text-[1.1rem] font-bold text-text mb-5">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-4 gap-[0.4rem]">
              {PRESETS.map(preset => (
                ...
                >
                  ...
                  />
                  <span
                    className={`text-[9px] text-center whitespace-nowrap tracking-[0.02em] ${activePreset === preset.name ? 'text-primary' : 'text-[#555]'}`}
                  >
                    {preset.name}
                  </span>
                </button>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2 flex flex-row items-center justify-between gap-3">
              ...
              <input
                type="color"
                aria-label="Fundo dos itens"
                value={formValues.item_bg_color}
                onChange={e => setColor('item_bg_color', e.target.value)}
