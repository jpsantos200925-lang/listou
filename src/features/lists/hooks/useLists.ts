...
type ListCreatePayload = Omit<List, 'id' | 'user_id' | 'created_at' | 'logo_url'>
...
export function useLists() {
  ...
  function migrateLegacyLogos(data: List[]) {
    data
      .filter(list => list.logo_url?.startsWith('data:'))
      .forEach(list => {
        migrateLogoToStorage(list.id, list.logo_url as string)
          .then(publicUrl =>
  ...
  const addList = useCallback(
    async (payload: ListCreatePayload, logoFile: File | null): Promise<List> => {
      ...
      if (logoFile) {
        try {
          ...
          console.warn(
            '[useLists] Logo upload falhou, criando lista sem logo:',
  ...
  const editList = useCallback(
    async (id: string, payload: Partial<List>, logoFile: File | null): Promise<List> => {
      ...
      if (logoFile) {
        try {
          ...
          console.warn(
            '[useLists] Logo upload falhou, salvando lista sem atualizar logo:',
            (err as Error).message
          )
        }
      }
